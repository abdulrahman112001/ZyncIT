...
${this.customData.serverResponse}`;
  ...
  var newSmsBtn = document.getElementById("newSmsBtn");
  var closeSmsModal = document.getElementById("closeSmsModal");
  var cancelSmsBtn = document.getElementById("cancelSmsBtn");
  var sendSmsBtn = document.getElementById("sendSmsBtn");
  var smsDevice = document.getElementById("smsDevice");
  var smsPhone = document.getElementById("smsPhone");
  var smsMessage = document.getElementById("smsMessage");
  var charCount = document.getElementById("charCount");
  var chatDeviceSelect = document.getElementById("chatDeviceSelect");
  ...
  function showMainUI() {
    ...
    userAvatar.src = currentUser.photoURL || "https://ui-avatars.com/api/?name=" + encodeURIComponent(currentUser.displayName || "U");
  ...
  signupBtn.addEventListener("click", async () => {
    const name5 = signupName.value.trim();
    const email = signupEmail.value.trim();
    const password = signupPassword.value;
    if (!name5 || !email || !password) {
      showToast("Please fill in all fields", "error");
      return;
    }
    showLoading();
  ...
  function renderDevices() {
    ...
    <div class="list-item ${device.isOnline ? "device-online" : ""}">
      ...
      <div class="list-item-content">
        ...
        <div class="list-item-subtitle">${device.platform} \u2022 ${device.isOnline ? "Online" : "Offline"}</div>
      </div>
  ...
  function updateDeviceSelects() {
    ...
    const options = mobileDevices.map((d) => `<option value="${d.id}">${d.name}</option>`).join("");
      ...
      const unsub = onSnapshot(
        ...
        (snapshot) => {
          ...
          snapshot.forEach((doc2) => {
            ...
            console.log("\u{1F4E9} SMS data:", doc2.id, "read:", data.read, data);
            messages.push({
              id: doc2.id,
              deviceId,
  ...
  async function loadNotifications() {
    ...
    deviceIds.forEach((deviceId) => {
      ...
      const unsub = onSnapshot(q2, (snapshot) => {
        const notifications = [];
        snapshot.forEach((doc2) => {
          notifications.push({ id: doc2.id, deviceId, ...doc2.data() });
        });
  ...
  function getNotificationIcon(type, appName) {
    ...
    if (appLower.includes("whatsapp")) return "\u{1F4F1}";
    if (appLower.includes("telegram")) return "\u2708\uFE0F";
    if (appLower.includes("messenger")) return "\u{1F4AC}";
    if (appLower.includes("mail") || appLower.includes("gmail")) return "\u{1F4E7}";
    if (appLower.includes("phone") || appLower.includes("call")) return "\u{1F4DE}";
    if (appLower.includes("message") || appLower.includes("sms")) return "\u{1F4AC}";
    if (appLower.includes("calendar")) return "\u{1F4C5}";
    if (type === "sms") return "\u{1F4AC}";
    if (type === "call") return "\u{1F4DE}";
    if (type === "whatsapp") return "\u{1F4F1}";
  ...
  function renderNotifications(notifications) {
    ...
    notificationsList.innerHTML = notifications.map(
      (notif) => `
    <div class="list-item notification-item notification-${notif.type || "other"}">
      <div class="list-item-icon notification-icon">
        ${getNotificationIcon(notif.type, notif.appName)}
      </div>
      <div class="list-item-content">
        <div class="list-item-title">${notif.title || notif.appName || "Notification"}</div>
  ...
  function renderSMS(messages) {
    ...
    smsList.innerHTML = conversations.map(
      (conv) => `
  ...
  function getInitials(name5) {
    if (!name5) return "?";
    const words = name5.trim().split(" ");
    if (words.length >= 2) {
  ...
  function showConversation(phoneNumber) {
    const conversation = allSMSMessages.filter((msg) => (msg.phoneNumber || msg.sender) === phoneNumber).sort((a, b) => (a.timestamp || 0) - (b.timestamp || 0));
    ...
    <div class="conversation-view">
      ...
      <div class="conversation-messages">
        ${conversation.map(
      (msg) => `
          <div class="message-bubble ${msg.direction === "outgoing" || msg.type === "sent" ? "sent" : "received"}">
  ...
  async function sendConversationMessage(phoneNumber, inputElement) {
    ...
    if (!message) return;
    console.log("\u{1F50D} Looking for Android device for user:", currentUser.uid);
    const devicesQuery = query(
      collection(db, "devices"),
      where("userId", "==", currentUser.uid)
    );
    const devicesSnapshot = await getDocs(devicesQuery);
    console.log("\u{1F4F1} Found devices:", devicesSnapshot.size);
    ...
    androidDevices.forEach((doc2) => {
      console.log("Device:", doc2.data().id, "lastSeen:", doc2.data().lastSeen);
    });
    ...
    console.log("\u{1F4E4} Sending SMS via device:", deviceId);
    try {
  ...
  var allCallsData = [];
  var currentCallConversation = null;
  function renderCalls(calls) {
    ...
    <div class="list-item call-group call-${group.lastCall.type}" data-phone="${group.phoneNumber}">
      ...
      <div class="list-item-meta">
        ...
        ${group.missedCount > 0 ? `<div class="list-item-badge missed">${group.missedCount}</div>` : ""}
      </div>
    </div>
  `
    ).join("");
  ...
  async function sendFileMessage(file) {
    ...
    try {
      ...
      await addDoc(collection(db, "chats"), {
        ...
        content: result.fileType === "image" ? "\u{1F4F7} Image" : `\u{1F4CE} ${result.fileName}`,
