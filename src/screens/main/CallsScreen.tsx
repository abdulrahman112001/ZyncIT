...
const SwipeableCallItem = ({
  ...
  return (
    <View style={[styles.swipeContainer, { backgroundColor: bgColor }]}>
      ...
      <Animated.View style={[styles.callRow, { backgroundColor: bgColor, transform: [{ translateX }] }]}>
        <TouchableOpacity
          onPress={() => {
            if (swiped) {
              resetSwipe();
          ...
          <View style={styles.callContent}>
            <View style={styles.topRow}>
              <View style={styles.nameRow}>
                <Text
                  style={[styles.callerName, { color: textColor }, isMissed && styles.missedCallName]}
                  numberOfLines={1}
                >
                  {displayName}
                </Text>
                {item.count > 1 && (
                  <Text style={[styles.callCount, { color: secondaryTextColor }]}>({item.count})</Text>
                )}
              </View>
            ...
            <View style={styles.subtitleRow}>
              ...
              <Text style={[styles.phoneText, { color: secondaryTextColor }]}>{item.phoneNumber}</Text>
            </View>
          </View>
        </TouchableOpacity>
      </Animated.View>
...
const CallsScreen = () => {
  ...
  const { calls, isLoading, loadCalls } = useCallStore();
  const { requestPermissions, startCallListener } = useNativeEvents();
  ...
  const initializeCallListener = useCallback(async () => {
    ...
    // Load existing calls from store/Firebase
    loadCalls();
  ...
  const renderTitle = () => (
    <View style={styles.titleContainer}>
      <Text style={[styles.title, { color: textColor }]}>{isRTL ? 'المكالمات' : 'Calls'}</Text>
    </View>
  );
